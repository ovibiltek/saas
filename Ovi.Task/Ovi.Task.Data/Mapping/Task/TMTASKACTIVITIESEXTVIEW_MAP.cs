...
{
    public sealed class TMTASKACTIVITIESEXTVIEW_MAP : ClassMap<TMTASKACTIVITIESEXTVIEW>
    {
        public TMTASKACTIVITIESEXTVIEW_MAP()
        {
            Id(x => x.TSA_ID);
            Map(x => x.TSA_TASK);
            ...
            Map(x => x.TSA_HIDDEN);
            Map(x => x.TSA_PRIVATE);
            Map(x => x.TSA_TASKSTATUS);
            Map(x => x.TSA_TASKSTATUSDESC).ReadOnly().Formula("dbo.GetDesc('TMSTATUSES','STA_DESC', 'TASK#' + TSA_TASKSTATUS, (SELECT s.STA_DESC FROM TMSTATUSES s WHERE s.STA_CODE = TSA_TASKSTATUS AND s.STA_ENTITY = 'TASK') ,:SessionContext.Language)");
            Map(x => x.TSA_TASKCUSTOMER);
            Map(x => x.TSA_TASKBRANCH);
            Map(x => x.TSA_TASKBRANCHDESC);
            Map(x => x.TSA_CHK01);
            Map(x => x.TSA_CHK02);
            Map(x => x.TSA_CHK03);
            Map(x => x.TSA_CHK04);
            ...
            Map(x => x.TSA_QUOTATIONSTATUSDESC).ReadOnly().Formula("(SELECT s.STA_DESC FROM TMSTATUSES s WHERE s.STA_ENTITY = 'QUOTATION' AND s.STA_CODE = TSA_QUOTATIONSTATUS)");
            Map(x => x.TSA_INVOICE);
            Map(x => x.TSA_QUOTATIONSTATUS);
            Map(x => x.TSA_TASKPCODE);
            Map(x => x.TSA_FILTER01);
            Map(x => x.TSA_FILTER06);
            Map(x => x.TSA_FILTER02);
            Map(x => x.TSA_FILTER03);
            Map(x => x.TSA_FILTER04);
            Map(x => x.TSA_FILTER05);
        }
    }
}