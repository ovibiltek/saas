...
namespace Ovi.Task.UI.Helper.Integration.Mikro
{
    public class SalesInvoiceHelper
    {
        ...
        public string GetMikroStr(TMSALESINVOICES invoice)
        {
            return SatisSiparis(invoice);
        }

        public MikroResultExt Transfer(TMSALESINVOICES invoice)
        {

            var postString = GetMikroStr(invoice);
            var url = ConfigurationManager.AppSettings["mikro_url"] + "/siparis/satis";
        ...
        public string SatisSiparis(TMSALESINVOICES invoice)
        {
            var customer = repositoryCustomers.Get(invoice.SIV_CUSTOMER);

            var lines = repositoryInvoices.ListMikroSalesLines(new GridRequest
            {
                loadall = true,
            ...
            {
                AdresNo = "1",
                CariKodu = customer.CUS_ACCOUNTCODE,
                DepoNo = "1",
                SiparisNo = invoice.SIV_CODE.ToString(),
                Tarih = invoice.SIV_CREATED.ToString(OviShared.ShortDate2, CultureInfo.InvariantCulture),
        ...
        private string StokKodu(TMMIKROSALESINVOICELINESVIEW line)
        {
        ...
        //SM007     PLATFORM
        private string SorumlulukMerkezi(TMMIKROSALESINVOICELINESVIEW line)
        {
        ...
        private string ProjeKodu(TMMIKROSALESINVOICELINESVIEW line)
        {
