...
function MenuEditor(idSelector, options) {
    ...
    this.delete = function (list, item) {

        var code = $(item).data('code');
        var codestr = code.toString();
        $('[data-id="' + codestr + '"]').show();
        item.remove();
    ...
    function createMenu(arrayItem, depth) {
        ...
        $.each(arrayItem, function (k, v) {
            ...
            var itemObject = { text: "", href: "", icon: "empty" };
            var temp = $.extend({}, v);
            if (isParent) {
            ...
            if (v.code != "") {
                var $span = $('<span>').addClass('txt').append(v.text + " - " + v.code).css('margin-right', '5px');
            }
            else {
    ...
    this.update = function () {
        ...
        if ($cEl.data("code") == "") {
            ...
            $cEl.find('span.txt').first().text($cEl.data('text') + " - " + $cEl.data("code"));
        }
    ...
    this.add = function () {
        ...
        $('[data-id="' + data.code + '"]').hide();
        var btnGroup = TButtonGroup();
        if (data.code != "") {
            var textItem = $('<span>').addClass('txt').text(data.text + " - " + data.code);
        }
        else {
